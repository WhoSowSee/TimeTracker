...
view_shift = (
	ceil((all_experiment_time + start_hour) // d) - 13.5
	if ceil(all_experiment_time / w) > 2
	else 0.5
)
...
ax[0][1].set_xticks(
	range(1, days + 1),
	[days_of_week[(i + start_day) % 7] for i in range(days)],
)
ax[0][1].set_xlim(view_shift, 15 + view_shift)
ax[0][1].xaxis.set_major_formatter(
	lambda x, _: days_of_week[(int(x - 0.5) + start_day) % 7]
)
